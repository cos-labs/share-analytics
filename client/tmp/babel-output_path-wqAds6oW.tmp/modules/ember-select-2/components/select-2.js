...
var Select2Component = Ember.Component.extend({
  ...
  didInsertElement: function didInsertElement() {
    ...
    options.formatResult = function (item) {
      ...
      if (id && description) {
        output += " <span class=\"text-muted\">" + Ember.Handlebars.Utils.escapeExpression(description) + "</span>";
      }
    ...
    options.initSelection = function (element, callback) {
      ...
      Ember.assert("select2#initSelection has been called without an \"" + "optionValuePath\" set.", optionValuePath);

      Ember.assert("select2#initSelection can not map string values to full objects " + "in typeahead mode. Please open a github issue if you have questions to this.", !self.get('_typeaheadMode'));
      ...
      if (unmatchedValues === 0) {
        ...
        Ember.warn("select2#initSelection was not able to map each \"" + optionValuePath + "\" to an object from \"content\". The remaining " + "keys are: " + values + ". The input will be disabled until a) the " + "desired objects is added to the \"content\" array or b) the " + "\"value\" is changed.", !values.length);
      }
    ...
    if (Ember.PromiseProxyMixin.detect(content)) {
      ...
      content.then(null, function (reason) {
        Ember.warn("select2: content promise was reject with reason " + reason + ". Recovering from this is not (yet) implemented.");
      });
    }
  ...
  valueChanged: function valueChanged() {
    ...
    if (Ember.PromiseProxyMixin.detect(value)) {
      ...
      value.then(function (value) {
        ...
        Ember.warn("select2: value promise was reject with reason " + reason + ". Recovering from this is not (yet) implemented.");
      });
    }
